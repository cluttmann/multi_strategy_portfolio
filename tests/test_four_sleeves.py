"""AAA / World-Trend / Mix8 ab 2026-09-23.
...
def test_target_allocation_is_25_25_50():
    assert main.strategy_allocations == {"aaa_allo": 0.25, "world_trend_allo": 0.25, "mix8_allo": 0.50}
...
def test_world_trend_state_machine_matches_backtest():
    rnd = random.Random(7)
...
def _daily(monkeypatch, broker, signals, stored_states):
    ...
    monkeypatch.setattr(main, "world_trend_signals", lambda cfg=None, today_iso=None: signals)
...
def test_daily_world_trend_data_error_means_no_orders(monkeypatch):
    ...
    def boom(cfg=None, today_iso=None):
        raise main.EodhdDataError("URTH.US: Live-Kurs ist 300 min alt")
    monkeypatch.setattr(main, "world_trend_signals", boom)
...
@pytest.mark.parametrize("values", [
    {"aaa": 2500.0, "world_trend": 2500.0, "mix8": 5000.0},   # im Ziel
    {"aaa": 3500.0, "world_trend": 2500.0, "mix8": 4000.0},   # AAA ueber, Mix8 unter
    {"aaa": 1500.0, "world_trend": 4000.0, "mix8": 4500.0},   # World ueber
    {"aaa": 4000.0, "world_trend": 4000.0, "mix8": 2000.0},   # Mix8 weit unter
    {"aaa": 3500.0, "world_trend": 2300.0, "mix8": 4200.0},   # AAA ueber, beide anderen unter
])
...
def test_tilt_order_follows_the_gap_to_target(monkeypatch):
    """AAA 35 % (ueber), World 23 % (knapp unter), Mix8 42 % (weit unter).
    Bei Kappe = Ziel bekam Mix8 hier nur 46,7 % und World 40,8 % - verkehrt."""
    vals = {"aaa": 3500.0, "world_trend": 2300.0, "mix8": 4200.0}
    monkeypatch.setattr(main, "get_all_strategy_values", lambda api: {**vals, "total": 10000.0})
    adj = main.calculate_rebalanced_allocations({})["adjusted_allocations"]
    assert adj["aaa_allo"] == pytest.approx(0.125)           # nur der Floor
...
def test_cloudbuild_deploys_and_schedules_the_new_sleeves():
    ...
    assert "--entry-point=daily_world_trend_route" in cb
    block = cb[cb.index("jobs describe daily_world_trend"):]
    block = block[:block.index("waitFor")]
    assert "jobs create http daily_world_trend" in block        # neuer Job: create-Zweig
