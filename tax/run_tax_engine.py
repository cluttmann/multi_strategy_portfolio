...
def print_wiso_summary(summaries: list[dict]) -> None:
    ...
    for s in summaries:
        ...
        print(f"  ║  Dividends (gross, before TFS):       {s['total_dividends_gross_eur']:>10} EUR")
        print(f"  ║  Dividends (after TFS):               {s['total_dividends_tfs_eur']:>10} EUR")
        print(f"  ║  Realized Gains (after TFS):          {s['total_realized_gains_tfs_eur']:>10} EUR")
        print(f"  ║  Realized Losses (after TFS):         {s['total_realized_losses_tfs_eur']:>10} EUR")
        print(f"  ║  Vorabpauschale (after TFS):          {s['vorabpauschale_eur']:>10} EUR")
        print(f"  ║  Interest Income:                     {s['interest_income_eur']:>10} EUR")
        print(f"  ║  Foreign Tax Paid (creditable):       {s['foreign_tax_paid_eur']:>10} EUR")
        print(f"  ║")
        print(f"  ║  WISO Anlage KAP Fields:")
        print(f"  ║  → Zeile 7 (Einkünfte):              {s['wiso_anlage_kap_zeile_7']:>10} EUR")
        print(f"  ║  → Zeile 12 (Verluste):              {s['wiso_anlage_kap_zeile_12']:>10} EUR")
        print(f"  ║  → Zeile 51 (ausl. Steuern):         {s['wiso_anlage_kap_zeile_51']:>10} EUR")
        print(f"  ║")
        print(f"  ║  ── Estimated German Tax ──────────────────────────────")
        print(f"  ║  Net taxable income:                  {s['net_taxable_income_eur']:>10} EUR")
        print(f"  ║  − Sparerpauschbetrag used:           {s['sparerpauschbetrag_used_eur']:>10} EUR")
        print(f"  ║  = Taxable after Freibetrag:          {s['taxable_after_freibetrag_eur']:>10} EUR")
        print(f"  ║  Abgeltungsteuer (25%):               {s['abgeltungsteuer_eur']:>10} EUR")
        print(f"  ║  + Solidaritätszuschlag (5.5%):       {s['solidaritaetszuschlag_eur']:>10} EUR")
        print(f"  ║  + Kirchensteuer:                     {s['kirchensteuer_eur']:>10} EUR")
        print(f"  ║  = Total German tax:                  {s['total_german_tax_eur']:>10} EUR")
        print(f"  ║  − Foreign tax credit:                {s['foreign_tax_credit_eur']:>10} EUR")
        print(f"  ║  ═══════════════════════════════════════════════════════")
        print(f"  ║  💰 TAX DUE TO FINANZAMT:             {s['tax_due_eur']:>10} EUR")
        print(f"  ║")
        print(f"  ╚═══════════════════════════════════════════════════════════╝")

    print("\n  ℹ  Note: Sparerpauschbetrag (€1,000) is applied per year assuming")
    print("      it is not used elsewhere. Kirchensteuer set to 0% (adjust in config).")
    print("      This is an estimate — WISO Steuer calculates the final amount.")
    print("=" * 70)
