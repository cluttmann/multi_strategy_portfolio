...
Modes:
  ...
  default            — read the latest already in each tab (max ID for
                       equity, max date for cash) and emit only newer
                       activities.
  --since YYYY-MM-DD — override: emit everything since this date for both
                       tabs.
...
def fetch_activities(api, activity_type: str, after_iso: str):
    ...
    while True:
        ...
        if not chunk:
            break
        out.extend(chunk)
        token = getattr(chunk, "next_page_token", None)
        if not token:
            break
        page_token = token
    return out
...
# -----------------------------------------------------------------------------
def fill_to_row(raw: dict, lookup: dict):
...
def div_to_row(raw: dict, lookup: dict):
    sym = resolve_symbol(raw, lookup)
    isin = SYMBOL_TO_ISIN.get(sym, "")
    if not isin:
        return None
...
def sync_equity(api, gc, since_override):
    ...
    if since_override:
        ...
        # Take max of both tabs — master log is the canonical source of truth,
        # but if the import tab somehow has a newer row (manual edit), respect it.
        ...
        try:
            since_date = datetime.strptime(max_id[:8], "%Y%m%d").date()
        except ValueError:
            sys.exit(f"[equity] Couldn't parse date prefix from ID {max_id!r}")
        print(f"[equity] Mode: max-id ({max_id} → fetching from {since_date} forward)")
    ...
    print(f"[equity]   FILL: {len(fills)}")
    divs = fetch_activities(api, "DIV", after_iso)
    print(f"[equity]   DIV:  {len(divs)}")

    all_acts = list(fills) + list(divs)
    lookup = build_symbol_lookup(all_acts)

    rows = []
    skipped_max_id = skipped_no_isin = skipped_other = 0
    unmapped_symbols = set()

    for act in all_acts:
        raw = act._raw
        aid = raw.get("id", "")
        if max_id and aid <= max_id:
            skipped_max_id += 1
            continue
        atype = raw.get("activity_type", "")
        if atype == "FILL":
            row = fill_to_row(raw, lookup)
        elif atype == "DIV":
            row = div_to_row(raw, lookup)
        else:
            row = None
        if row is None:
            sym = resolve_symbol(raw, lookup)
    ...
    if skipped_max_id:
        print(f"[equity]   skipped (already in sheet via max-id): {skipped_max_id}")
    if skipped_no_isin:
    ...
    # Append new rows to the master log, deduped by ID so --since runs don't
    # double-write entries already present.
    if rows:
        existing_ids = set(v.strip() for v in ws_master.col_values(1)[1:] if v.strip())
        fresh = [r for r in rows if r[0] not in existing_ids]
        if fresh:
            print(f"[equity] Appending {len(fresh)} row(s) to master log '{EQUITY_MASTER_TAB}'…")
            ws_master.append_rows(fresh, value_input_option="USER_ENTERED")
            print(f"[equity] Master log updated.")
        else:
            print(f"[equity] Master log already contains all {len(rows)} row(s).")
...
def sync_cash(api, gc, since_override):
    ...
    if since_override:
        ...
        # Take max of both tabs — master log is canonical, but if the import
        # tab somehow has a newer row (manual edit), respect it.
        ...
        since_date = max_date
        print(f"[cash] Mode: max-date in sheets = {max_date}; emitting activities strictly newer")
    ...
    print(f"[cash]   CSD: {len(csds)}  CSW: {len(csws)}  INT: {len(ints)}  INTNRA: {len(intnras)}  FEE: {len(fees)}")

    # Sum same-day FEE.net_amount (absolute) into a date → fee map.
    fees_by_date_iso = {}
    for f in fees:
        raw = f._raw
        d = raw.get("date") or (raw.get("transaction_time") or "")[:10]
        if not d:
            continue
        try:
            fee_amt = abs(float(raw.get("net_amount") or 0))
        except ValueError:
            continue
        fees_by_date_iso[d[:10]] = fees_by_date_iso.get(d[:10], 0.0) + fee_amt

    rows = []
    skipped_old = skipped_other = 0
    for act in list(csds) + list(csws) + list(ints) + list(intnras):
        raw = act._raw
        atype = raw.get("activity_type", "")
        ...
        if act_date is None:
            skipped_other += 1
            continue
        if max_date is not None and act_date <= max_date:
            skipped_old += 1
            continue
        try:
        ...
        if atype == "CSD":
            type_str = "TransferIn"
            amount = net
            fee_amt = fees_by_date_iso.get(date_iso[:10], 0.0)
        elif atype == "CSW":
            type_str = "TransferOut"
            amount = abs(net)
            fee_amt = fees_by_date_iso.get(date_iso[:10], 0.0)
        elif atype in ("INT", "INTNRA"):
            type_str = "Interest"
            amount = net  # INT positive, INTNRA already-signed (typically negative)
            fee_amt = 0.0
        else:
            skipped_other += 1
            continue
    ...
    if skipped_old:
        print(f"[cash]   skipped (date ≤ max in sheet): {skipped_old}")
    if skipped_other:
        print(f"[cash]   skipped (malformed/unknown subtype): {skipped_other}")

    _write_tab(ws_import, CASH_HEADERS, rows, "cash")

    # Append to master log, deduped by full-row fingerprint (no IDs on cash rows).
    if rows:
        existing_fingerprints = {
            tuple(r) for r in ws_master.get_all_values()[1:] if any(c.strip() for c in r)
        }
        fresh = [r for r in rows if tuple(r) not in existing_fingerprints]
        if fresh:
            print(f"[cash] Appending {len(fresh)} row(s) to master log '{CASH_MASTER_TAB}'…")
            ws_master.append_rows(fresh, value_input_option="USER_ENTERED")
            print(f"[cash] Master log updated.")
        else:
            print(f"[cash] Master log already contains all {len(rows)} row(s).")
